...
export const releaseInitialWhitelistPayment = functions.pubsub.schedule('* */23 * * *').timeZone('America/New_York')
  .onRun(async (context) => {
...
export const releaseMonthlyWhitelistPayment = functions.pubsub.schedule('*/15 * * * *').timeZone('America/New_York')
  .onRun(async (context) => {
    const contract = getCacttTokenContract();
